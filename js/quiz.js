let quizSession = null;
        ...
        function startWeakFocusQuiz() {
            // [냐냐 PATCH] '오늘의 복습'과 동일한 기준(weakScore>=1)으로 단어를 모음
            const reviewWords = getTodayReviewWords();
            if (reviewWords.length < 2) {
                showToast("복습할 약점 단어가 2개 이상 있어야 해요! 퀴즈를 풀다 보면 쌓여요.", "info");
                return;
            }
            changeTab('quiz');
            setTimeout(() => {
                // 복습 전용 풀을 직접 지정해서 퀴즈 시작
                quizReviewPoolOverride = reviewWords;
                startQuiz();
            }, 50);
        }
