let vocabulary = [];
        ...
        function diaryTabsHtml(ds, log, what) {
            ...
            const cell = (k, c, color) => {
                ...
                //   ⚠️ 클래스(rounded-br-2xl · ring-violet-500/60)로는 안 먹는다 —
                //      태풍은 처음에 본 클래스만 만들어 둔다. 모서리도 테두리도 직접 적는다.
        ...
        // [냐냐 기준 2026-09-07] 단어와 똑같이 간다.
        //   후퇴 — **어디서 틀리든** 한 칸 뒤로. 첨삭 네 모드, 빈칸 70% 미만.
        //     곡선 밖이었으면 0단계로 들여놓는다 (예전 grammarReviewEnter 가 하던 일).
        //     하루에 한 칸까지만 — 같은 오답 하나에 여러 번 불려도 두 칸 안 떨어진다.
        //   전진 — **복습 배너로 시작한 그 미션에서만.** 한 번 잘 썼다고 곡선이 나가면
        ...
        //   문법은 lastWrongDate 하나로는 놓치는 게 있다 — 하루에 한 번만 물리니까,
        //   오늘 이미 물린 뒤에 또 틀린 건 날짜가 안 바뀐다.
        ...
        // ============================================================
        //   [냐냐 요청] 덤은 +2 → +1 (2026-09-21). 물어본 낱말의 정답 점수는 그대로다 —
        //   한 문제에 +4 가 움직이던 것을 +3 으로 줄인다.
        const SYNONYM_AWARD = 1;
        ...
        //   찾아본 건 내가 표시하면 되니 네 모드 모두 제대로 쓰면 +2 로 통일한다.
        ...
        function buildHelpHtml() {
            ...
            const SCORE_TABLE = [
                { group: '퀴즈', color: 'text-indigo-600 bg-indigo-50 border-indigo-200', rows: [
                    ...
                    ['주관식 · 내가 쓴 <b>그 유의어 낱말</b>에도 덤 <span class="text-[10px] font-semibold text-slate-400">(악센트까지 같을 때만)</span>', '+1', '—'],
                    ['동사 활용형', '+2', '−2'],
                ...
                { group: '복습', color: 'text-amber-600 bg-amber-50 border-amber-200', rows: [
                    ...
                    ['쓰기 복습 · 내가 쓴 <b>그 유의어 낱말</b>에도 덤', '+1', '—'],
                    ['쓰기 복습 · 익힌 뒤 3바퀴에서', '−1.5', '−2.5'],
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-4 space-y-3">
                ...
                <p class="text-[11px] text-slate-500 font-semibold leading-relaxed">
                    ...
                    점수가 이상하면 첨삭 결과에서 <b>↺ 를 눌러 점수를 직접 바꾸면</b> 돼요 — <b>+2 → 0 → −2</b> 로 돌아요.
                    줄은 점수순으로 놓이고, 점수를 바꿔도 자리는 그대로예요.
                </p>
            </div>
            ...
            <div class="bg-[#eef5fb] rounded-2xl border border-[#c3d9ec] p-4 space-y-3">
                ...
                <p class="text-[11px] text-[#2c5578] font-semibold leading-relaxed">
                    첨삭은 <b>노트 제목이 아니라 노트에 적힌 것</b>을 읽고 판단해요. 노트마다 한 줄씩
                    <b>표 제목 + 내가 쓴 설명(400자까지) + 표 안의 스페인어</b>가 넘어가요. 제목이 넓어도(예: '위치를 나타내는 표현'
                    인데 내용은 al/del 축약) 적어둔 내용이 좁으면 엉뚱한 문장에 안 걸려요.
                    <b>판정이 어긋나면 노트를 고치면 돼요</b> — 특히 <b>표 제목</b>이 그 표가 뭘 다루는지 말해주는 유일한 줄이라,
                    비어 있으면 문법 탭 위에 노란 줄로 알려줘요.
                </p>
                <p class="text-[11px] text-[#2c5578] font-semibold leading-relaxed">
                    <b>동사 꼴은 노트와 상관없이 앱이 직접 잡아요.</b> 내 단어장의 동사 활용표를 보고 문장이 쓴 꼴을
                    알아내서(현재분사·과거분사·각 시제), 그 꼴을 가르치는 노트를 제목으로 찾아 같이 짚어요.
                    <b>현재진행(estar+현재분사)</b>과 <b>현재완료(haber+과거분사)</b>처럼 두 조각짜리는 조각이 다 있을 때만요.
                    AI는 한 문장이 노트 둘을 동시에 쓰면 하나만 고르는 버릇이 있어서, 그 자리를 앱이 메워요.
                </p>
                <p class="text-[11px] text-[#2c5578] font-semibold leading-relaxed">
            ...
            <div class="bg-violet-50 rounded-2xl border border-violet-200 p-4 space-y-3">
                ...
                <p class="text-xs text-violet-900 font-semibold leading-relaxed">
                    ...
                    헤더의 <b>📖 복습</b> 버튼을 누르면 <b>쓰기 복습</b>으로 진행해요 (몇 번에 나눠서 할지 고르고, <b>한 번에 몇 개씩</b>인지는 그 창의 톱니바퀴에서 바꿀 수 있어요 — 기본 100개):
                    먼저 <b>가린 채 1번</b> 써서 아는지 확인하고(맞히면 여기서 끝),
                    틀린 것만 <b>보면서 2번</b> 쓴 다음 <b>순서를 섞어 다시 한 번</b> 확인해요.
                    ${/* [냐냐 지적] 예전 문구가 실제와 달랐다 (2026-09-22) — "중간에 그만두면 안 한 걸로
                         남아요" 라고 적혀 있었는데, 실제로는 문제를 푸는 그 자리에서 반영된다.
                         냐냐님이 "귀찮아서 그냥 꺼도 인정되는 게 마음에 걸렸다" 고 하셔서 사실대로 고친다. */''}
                    <b>망각곡선은 문제를 푸는 그 자리에서</b> 움직여요 — 1바퀴에 맞히면 한 칸 앞으로,
                    틀리면 한 칸 뒤로 가고 오늘 줄에서 빠져요 (틀린 건 곡선이 0칸으로 내려가 <b>내일 다시</b> 나와요).
                    그래서 <b>중간에 그만둬도 이미 푼 것은 그대로 인정</b>되고, 안 푼 것만 오늘 줄에 남아요.
                    3바퀴는 점수만 조금 달라져요 — 익혀서 맞히면 −1.5, 끝내 틀리면 −2.5.
                </p>
                <table class="w-full text-xs bg-white rounded-xl overflow-hidden">
                    <thead>
        ...
        //   ⚠️ 단어는 **등록하는 그 순간** 물어서 채운다. 카드를 펼칠 때도 없으면 한 번 더 물어본다
        //      (키가 없었거나 그때 실패한 것을 위한 그물). 한 번 물으면 단어에 적어두니 두 번 묻지 않는다.
        //      이미 있던 1372개는 2026-09-15 에 묶음으로 한 번에 채웠다.
        // ============================================================
...
<!-- [냐냐 요청] 단어 찾기(돋보기)는 표 하나가 아니라 전체에 걸리는 모드라, 카드에서 빼고
        ...
        //   ⚠️ 머리 줄 폭 때문에 짧은 표현만 넣는다 (15글자 안쪽). 실제로 외치는 말만.
        //   ⚠️ 앞의 10줄 순서는 바꾸지 말 것 — 9/23 = Poco a poco 부터 하루씩 센다.
        //   냐냐님이 여자라 형용사는 여성형으로 (Tranquila · Bienvenida · Campeona …)
        // ============================================================
        const DAILY_TITLES = [
            ['Poco a poco', '조금씩 조금씩'],
            ['Paso a paso', '한 걸음씩'],
            ...
            ['Respira', '숨 한번 쉬고!'],
            ['Enhorabuena', '축하해!'],
            ['Cada día mejor', '날마다 더 나아져!'],
            ['Vale', '좋아! (스페인 말투)'],
            ['Qué pasada', '대박! (스페인 말투)'],
            ['A estudiar', '공부하자!'],
            ['Un abrazo', '꼭 안아줄게!'],
            ['Tú sí puedes', '너라면 할 수 있어!'],
            ['Disfruta', '즐겨!'],
            ['Madre mía', '세상에!'],
            ['Con todo', '온 힘을 다해!'],
            ['Mil gracias', '정말 고마워!'],
            ...
            ['Por supuesto', '물론이지!'],
            ['Campeona', '챔피언!'],
            ['Vas muy bien', '아주 잘하고 있어!'],
            ['Guau', '와!'],
            ['Eres capaz', '넌 해낼 수 있어!'],
            ['Buen finde', '좋은 주말! (주말 줄임말)'],
            ['Felicidades', '축하해요!'],
            ['Dale', '가보자! (중남미 말투)'],
            ['Qué lista', '똑똑하다!'],
            ['Nos vemos', '또 봐!'],
            ['Viva', '만세!'],
            ['Buenísimo', '최고야!'],
            ['Pura vida', '인생 최고! (코스타리카 인사)'],
            ['Maravillosa', '멋져!'],
            ['Sí se puede', '할 수 있다! (응원 구호)'],
            ['Hasta mañana', '내일 봐!'],
            ['Chévere', '멋져! (중남미 말투)'],
            ['Qué fuerte', '대박!'],
            ['Salud', '건배!'],
            ['Qué padre', '멋지다! (멕시코 말투)'],
            ['Échale ganas', '힘내! (멕시코 말투)'],
            ['Buena onda', '좋은 기운! (중남미 말투)'],
            ['Un besito', '쪽!'],
            ['Hasta luego', '이따 봐!'],
            ['Aprende jugando', '놀면서 배우자!'],
            ['Buen provecho', '맛있게 먹어!'],
            ['A darle', '가보자!'],
            ['Gracias', '고마워!'],
            ['Te quiero', '사랑해!'],
            ['De nada', '천만에!'],
            ['Tú lo vales', '넌 그만한 가치가 있어!'],
            ['Qué maja', '착하다! (스페인 말투)'],
