        ...
        function renderWordList() {
            ...
            filteredSorted.forEach(w => {
                ...
                <div class="rounded-3xl p-5 ${cardStyle} flex flex-col justify-between hover:shadow-md transition-all duration-300 relative group gap-4">
                    ...
                    <!-- 핵심만 정리된 노트 -->
                    ${w.notes ? `<div class="bg-amber-50/50 p-2.5 rounded-2xl border border-amber-200/50 text-xs text-amber-900 leading-snug whitespace-pre-line font-bold -mt-4"><span class="font-bold text-amber-800 block text-[9px] uppercase tracking-wider mb-0.5"><i class="fa-solid fa-thumbtack text-[8px]"></i> 핵심 노트</span>${w.notes}</div>` : ''}
                </div>
                `;
            });
