// ============================================================
        ...
        function getGameHighScore(gameType) {
            try { return parseInt(localStorage.getItem('demo_game_hs_' + gameType) || '0', 10) || 0; }
            catch (e) { return 0; }
        }
        function getGameWeekHighScore(gameType) {
            try {
                const raw = localStorage.getItem('demo_game_whs_' + gameType);
                if (!raw) return 0;
        ...
        function collectGameHighScores() {
            ...
            GAME_TYPES.forEach(g => {
                ...
                let week = null;
                try {
                    const raw = localStorage.getItem('demo_game_whs_' + g);
        ...
        function mergeGameHighScores(remote) {
            ...
            GAME_TYPES.forEach(g => {
                ...
                if ((r.all || 0) > getGameHighScore(g)) {
                    try { localStorage.setItem('demo_game_hs_' + g, String(r.all)); } catch (e) {}
                }
                // 이번 주: 같은 주차일 때만, 그리고 큰 쪽을 남긴다
                if (r.week && r.week.week === getWeekKey() && (r.week.score || 0) > getGameWeekHighScore(g)) {
                    try { localStorage.setItem('demo_game_whs_' + g, JSON.stringify(r.week)); } catch (e) {}
                }
            });
        }
        ...
        function setGameHighScore(gameType, score) {
            ...
            if (score > prev) {
                try { localStorage.setItem('demo_game_hs_' + gameType, String(score)); } catch (e) {}
                isNewAllTime = true;
            }
            // 이번 주 최고
            ...
            if (score > weekPrev) {
                try { localStorage.setItem('demo_game_whs_' + gameType, JSON.stringify({ week: getWeekKey(), score })); } catch (e) {}
                changed = true;
            }
