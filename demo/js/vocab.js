...
Also classify the irregularity as EXACTLY one of: ${irregularTypesFor(tense).map(t => `"${t}"`).join(', ')} ("none" = fully regular).`;
        ...
        function saveFilterPrefs() {
            try {
                localStorage.setItem('demo_word_filters', JSON.stringify({
        ...
        function loadFilterPrefs() {
            try {
                const raw = localStorage.getItem('demo_word_filters');
        ...
        function saveDisplayPrefs() {
            try { localStorage.setItem('demo_word_display', JSON.stringify(displayPrefs)); } catch (e) {}
        }
        function loadDisplayPrefs() {
            try {
                const raw = localStorage.getItem('demo_word_display');
                if (raw) {
